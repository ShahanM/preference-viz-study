...
const LeftPanel: React.FC = () => {
    ...
    const [textResponse, setTextResponse] = useState<Map<string, string>>(new Map<string, string>());
        ...
        <Container className="leftpanel">
            ...
            <Row>
                <p className="mt-3" style={{ fontWeight: '500' }}>
                    Please use this page to collect some notes for the essay that you will write in the next step of the
                    study. To help you write the essay, here are some guiding prompts, and questions.
                </p>
            </Row>
            <hr />
            {PROMPTS && PROMPTS.length > 0 ? (
                PROMPTS.map((prompt, idx) => (
                    <div key={`prompt-row-${idx}`} style={{ textAlign: 'left', padding: '0 0.5rem' }}>
                        {Parse(prompt.text)}
                        {/* <p>
                        ...
                        <Form>
                            <Form.Group className="mb-1" controlId="uniquePreferences">
                                <Form.Control
                                    ...
                                    onChange={(e) => updateTextResponse(prompt.context_tag, e.target.value)}
                                />
                            </Form.Group>
                        </Form>
                    </div>
            ...
            <Row className="mt-1 p-3">
                <Button
                    variant={'ers'}
                    size="lg"
                    className="layout-footer-btn"
                    onClick={handleSubmit}
                    disabled={submitButtonDisabled || loading || dataSubmitted}
                >
