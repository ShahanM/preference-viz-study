...
const MovieGridItem = memo(
    ({ movieItem, ratingCallback }: MovieGridItemProps) => {
        ...
        return (
            <li id={'TN_' + movieItem.id} className={'grid-item'}>
                <Image
                    className="grid-item-image rounded"
                    height="200px"
                    width="140px"
                ...
                <div className="grid-item-label">{movieItem.title + ' (' + movieItem.year + ')'}</div>
            </li>
        );
    },
    (prevProps, nextProps) => {
        return prevProps.movieItem === nextProps.movieItem && prevProps.ratingCallback === nextProps.ratingCallback;
    }
);
