...
import { type Movie } from './moviegriditem/MovieGridItem.types';
...
const MovieGrid: React.FC<MovieGridProps> = ({ itemsPerPage, dataCallback }) => {
    ...
    const rateMovies = useCallback(
        (newRating: number, movieid: string) => {
            setMovieMap((prevMovieMap) => {
    ...
    const visibleMovies = useMemo(() => {
        return [...movieMap.values()].slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
            ...
            <Row className="galleryFooter">
                <Col>
                    <div className="btnDiv">
                        <Button id="gallery-left-btn" disabled={prevBtnDisabled} variant="ers" onClick={renderPrev}>
                            &lt;
                        </Button>
                    </div>
                </Col>
                <Col>
                    <div className="btnDiv">
                        <Button id="gallery-right-btn" disabled={nextBtnDisabled} variant="ers" onClick={renderNext}>
                            {isLoadingMovies && nextBtnDisabled ? <LoadingText text={'Fetching more movies'} /> : '>'}
                        </Button>
                    </div>
                </Col>
            </Row>
