...
import type { VizDataProps, VisualizationProps } from './VisualizationTypes.types';
...
const DiscreteCoupled: React.FC<VisualizationProps> = ({
    ...
    const filteredData = useMemo(() => {
        return {
            myLikesCommLike: simNodeData.filter((d) => d.user_score >= likeCuttoff && d.community_score >= likeCuttoff),
            myDislikesCommLikes: simNodeData.filter(
                (d) => d.user_score < dislikeCuttoff && d.community_score >= likeCuttoff
            ),
            myLikesCommDislikes: simNodeData.filter(
                (d) => d.user_score >= likeCuttoff && d.community_score < dislikeCuttoff
            ),
            myDislikesCommDislikes: simNodeData.filter(
        ...
        if (currentSvgRefs.length === dataArrays.length) {
            ...
            svgRefs.current.forEach((svgRef, i) => {
                ...
                g.selectAll<SVGImageElement, VizDataProps>('image')
                    ...
                    .on('mouseover', (_event, d: VizDataProps) => {
                        d3.selectAll('image')
                            .filter(function () {
                    ...
                    .on('mouseout', (_event, d: VizDataProps) => {
                        d3.selectAll('image')
                            .filter(function () {
                                return d3.select(this).attr('data-id') === d.id;
                            })
                ...
                const simulation = d3
                    .forceSimulation<VizDataProps>()
                    .force(
                        'center',
                        d3
                            .forceCenter<VizDataProps>()
                            .x(svgWidth / 2)
                            .y(svgHeight / 2)
                    )
                    .force('charge', d3.forceManyBody<VizDataProps>().strength(0.5))
                    .force('collide', d3.forceCollide<VizDataProps>().strength(0.01).radius(45).iterations(5));
        ...
        <div className="flex w-full">
            ...
            <div className="flex-1">
                <div className="flex flex-row">
                    <div className="discrete-viz-container">
                        {' '}
                        {/* 3x3 Grid */}
                        {/* Row Headers */}
                        ...
                        <div className="row-1-col-3 discrete-viz-content-box">
                            ...
                            <svg ref={setSvgRef(2)} width={width / 2} height={height / 2}></svg>
                        </div>
                        <div className="row-2-col-3 discrete-viz-content-box">
                            {/* Canvas 4: My Dislikes & Community Dislikes */}
                            <svg ref={setSvgRef(3)} width={width / 2} height={height / 2}></svg>
                        </div>
                        <div className="row-3-col-2 discrete-viz-label-container">Likes</div>
                        <div className="row-3-col-3 discrete-viz-label-container">Dislikes</div>
                    </div>
                </div>
                <div className="viz-label-label-h flex flex-row">Everyone else</div>
            </div>
        </div>
    );
};
